    ...
    try {
      if (_isEdit) {
        ...
        final patch = <String, dynamic>{
          ...
          'color': _color,
          'isActive': _active,
          if (_workEnvironment != 'unspecified')
        ...
        if (!mounted) return;
        if ((_workEnvironment != 'unspecified' &&
                updated.workEnvironment != _workEnvironment) ||
            updated.weatherSensitive != _weatherSensitive) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(
            l.localeName.startsWith('es')
                ? 'El servidor no ha confirmado el entorno seleccionado. El cambio no se puede dar por guardado.'
                : 'The server did not confirm the selected environment. This change cannot be confirmed as saved.',
          )));
          return;
        }
              ...
              items: [
                if (_workEnvironment == 'unspecified')
                  DropdownMenuItem(
                      value: 'unspecified',
                      enabled: false,
                      child: Text(isEs ? 'Sin especificar' : 'Unspecified')),
                DropdownMenuItem(
