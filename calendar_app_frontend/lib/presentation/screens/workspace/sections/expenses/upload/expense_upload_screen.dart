...
import 'package:hexora/presentation/screens/workspace/sections/expenses/upload/operations/expense_operations.dart';
import 'package:hexora/presentation/screens/workspace/sections/expenses/upload/operations/form_helpers.dart';
import 'package:hexora/presentation/screens/workspace/sections/expenses/upload/operations/provider_operations.dart';
import 'package:hexora/presentation/screens/workspace/sections/expenses/upload/expense_upload_sections.dart';
  ...
  Future<void> _persistBatchJobMapping() async {
    ...
    await OcrImportJobMappingStore.instance.upsert(
      OcrImportJobMapping(
        ...
        groupId: _batchJobCacheKey(),
      ),
    );
  }

  Future<bool> _restorePersistedBatchJobMapping() async {
    final jobs = await OcrImportJobMappingStore.instance.loadForScope(
      userId: context.read<AuthProvider>().currentUser?.id ?? '',
      groupId: _batchJobCacheKey(),
    );
