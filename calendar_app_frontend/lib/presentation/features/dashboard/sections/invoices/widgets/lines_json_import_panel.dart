...
class _LinesJsonImportPanelState extends State<LinesJsonImportPanel> {
  ...
  void _repairJsonText(BuildContext context) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content:
            Text('JSON reparado. Revisa el resultado antes de importarlo.'),
      ),
    );
  }
  ...
  int? _detectLineCount(String input) {
    ...
    try {
      ...
      if (decoded is Map && decoded['lines'] is List) {
        return (decoded['lines'] as List).length;
      }
      if (decoded is Map && decoded['items'] is List) {
        return (decoded['items'] as List).length;
      }
      if (decoded is Map && decoded['concepts'] is List) {
        return (decoded['concepts'] as List).length;
      }
  ...
  Widget build(BuildContext context) {
    ...
    final textValidationError = !hasJsonText
        ...
        : (!isJsonTextValid
            ...
            : null);
    final lineCount =
        _useJsonFileMode ? null : _detectLineCount(_jsonTextCtrl.text);
    final canSubmitCurrentMode = _useJsonFileMode
        ? (canSubmit && hasJsonFile)
        : (canSubmit && hasJsonText);
              ...
              if (_useJsonFileMode) ...[
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(
                        widget.formatHint ?? 'Formato: {"draftLines": [...]}',
                        style: TextStyle(
                          fontFamily: 'monospace',
                          fontSize: 10,
                    ...
                    if (lineCount != null)
                      ...
                      MouseRegion(
                        ...
                        child: GestureDetector(
                          onTap:
                              widget.loadingPrompt ? null : widget.onCopyPrompt,
              ...
              Row(
                children: [
                  ...
                  Text(
                    ...
                    style:
                        t.bodySmall.copyWith(color: cs.onSurface, fontSize: 12),
                  ),
                  const SizedBox(width: 4),
                  Transform.scale(
                    scale: 0.78,
