...
import 'package:hexora/models/service/service.dart';
...
import 'package:hexora/data/statements/statements_api.dart';
import 'package:hexora/presentation/features/dashboard/sections/enable_banking/statements/statements_controller.dart';
import 'package:hexora/presentation/features/dashboard/sections/enable_banking/statements/statements_formatters.dart';
import 'package:hexora/presentation/features/dashboard/sections/enable_banking/statements/statements_shared.dart';
...
class _InsightsChatRuntime extends ChangeNotifier {
  ...
  Future<void> ensureLoaded(BuildContext context) async {
    ...
    if (_bootstraped && _historyKey == nextHistoryKey) return;
...
class _InsightsChatSheetState extends State<_InsightsChatSheet> {
  ...
  Widget _buildEventPreviewBubble(
    ...
    final untilDate =
        _eventDate(_safeMap(preview?['recurrence_rule'])?['untilDate']);
    final missing = _stringList(preview?['missing']);
...
class _InsightsEventEditDialogState extends State<_InsightsEventEditDialog> {
  ...
  void initState() {
    ...
    final rule = _safeMap(_preview['recurrence_rule']);
    _untilDate =
  ...
  void _save() {
    ...
    final previewRule = _safeMap(_preview['recurrence_rule']);
    final payloadRule = _safeMap(_payload['recurrence_rule']);
    if (previewRule != null || payloadRule != null) {
      ...
      _preview['recurrence_rule'] = nextPreviewRule;
      _payload['recurrence_rule'] = nextPayloadRule;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final recurrenceType =
        _safeMap(_preview['recurrence_rule'])?['recurrenceType']?.toString() ??
            '';
