...
import 'package:hexora/presentation/features/notifications/show_notifications/utils/notification_payload_helper.dart';
...
NotifCategoryMeta resolveActionMeta(String? action, ColorScheme cs) {
  switch (action) {
    ...
    case 'assigned':
      return NotifCategoryMeta(
          icon: Icons.assignment_ind_rounded, color: cs.primary);
    case 'updated':
      return NotifCategoryMeta(
    ...
    default:
      return NotifCategoryMeta(icon: Icons.event_rounded, color: cs.primary);
  }
}
...
String actionLabel(String? action, {required bool isEs}) {
  switch (action) {
    ...
    case 'assigned':
      return isEs ? 'Asignado' : 'Assigned';
    case 'updated':
      return isEs ? 'Actualizado' : 'Updated';
    case 'deleted':
      return isEs ? 'Eliminado' : 'Deleted';
    case 'reminder':
      return isEs ? 'Recordatorio' : 'Reminder';
    case 'started':
      return isEs ? 'Iniciado' : 'Started';
    case 'recurrence_added':
      return isEs ? 'Recurrencia' : 'Recurrence';
    case 'marked_done':
      return isEs ? 'Completado' : 'Completed';
    case 'reopened':
      return isEs ? 'Reabierto' : 'Reopened';
    default:
      return action ?? '';
  }
}
...
NotifCategoryMeta resolveStatusMeta(String? status, ColorScheme cs) {
  switch (status) {
    ...
    case 'cancelled':
      return NotifCategoryMeta(icon: Icons.cancel_rounded, color: cs.error);
    case 'completed':
      return NotifCategoryMeta(
...
String statusLabel(String? status, {required bool isEs}) {
  switch (status) {
    case 'confirmed':
      return isEs ? 'Confirmado' : 'Confirmed';
    case 'cancelled':
...
NotifCategoryMeta resolveEventTypeMeta(String? eventType, ColorScheme cs) {
  switch (eventType) {
    case 'work_visit':
      return NotifCategoryMeta(
          icon: Icons.handyman_rounded, color: cs.secondary);
    case 'meeting':
      return NotifCategoryMeta(icon: Icons.people_rounded, color: cs.primary);
    case 'appointment':
      return NotifCategoryMeta(
          icon: Icons.calendar_today_rounded, color: Colors.indigo.shade400);
    default:
      return NotifCategoryMeta(
...
String eventTypeLabel(String? eventType, {required bool isEs}) {
  switch (eventType) {
    case 'work_visit':
      return isEs ? 'Visita de trabajo' : 'Work visit';
    case 'meeting':
      return isEs ? 'Reunión' : 'Meeting';
    case 'appointment':
